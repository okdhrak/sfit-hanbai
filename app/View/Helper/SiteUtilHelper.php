...
class SiteUtilHelper extends AppHelper {
	...
	//プロパティの定義
	protected $maxQuant = 10;//pegenateソートボックスを表示する件数
	...
	public function getFavBtn($id) {
		...
		if ($this->_chkIssetCookie($id)) {//あり
			return sprintf('<a id="%s" href="javascript:;" class="fav" data-id="%s" data-status="%s"><img src="%s"></a>', $id, $id, 1, '/img/content_images/btn-favorite_done.png');
		} else {//なし
			return sprintf('<a id="%s" href="javascript:;" class="fav" data-id="%s" data-status="%s"><img src="%s"></a>', $id, $id, 0, '/img/content_images/btn-favorite_add.png');
		}
	}
...
$str = <<< EOF
<div id="selectedStations" class="unit04">
...
EOF;
	...
	//一覧ページサムネイル画像を取得
	public function getThumbPhoto($id, $cate, $width=300, $height=180) {
		if ($file_dir_re = opendir($file_dir = $this->_getFileDir($id))) {
			while (($file = readdir($file_dir_re)) !== false) {
				if (preg_match("/^{$cate}.+.jpg/", $file)) {
					return  sprintf('<img src="/searches/c_thumb_image/%s/%s/%s/%s" width="%s"  height="%s">', $id, $cate, $width, $height, $width, $height);
				}
			}
		}
		return sprintf('<img src="%s" width="%s" height="%s">', '/img/sample_list_300_180.png', $width, $height);
	}
	
	//詳細ページ画像を取得
	public function getDetailPhoto($id, $cate, $width=800, $height=530) {
		
		$result = NULL;
		$pr_width = ($cate == 'o') ? 226 : 350;
		
		if ($file_dir_re = opendir($file_dir = $this->_getFileDir($id))) {
			while (($file = readdir($file_dir_re)) !== false) {
				if (preg_match("/^{$cate}.+.jpg/", $file)) {
					$result .= sprintf('<div><p><a href="%s" rel="colorbox" title="" class="photoModal"><img src="%s" width="%s"></a></p><p class="enlarge"><img src="/img/content_images/enlarge.png" alt="enlarge"></p></div>', "/searches/c_detail_image/$id/$file/$width/$height", "/searches/c_detail_image/$id/$file/$width/$height", $pr_width);
				}
			}
			...
			if ($result !== NULL) {
				...
				return sprintf('<div><p><img src="%s" width="%s"></p></div>', '/img/sample_list_800_530.png', $pr_width);
			}
		}
	}
	...
	//物件の件数をチェックし一定件数以上であればtrueを返す
	public function chkArtQuant($array) {
		if (count($array) > $this->maxQuant) {
			return true;
		} else {
			return false;
		}
